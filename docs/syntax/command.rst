...
this process disregards quotes or symbols that have meaning in ``bash``.
...
may be evaluated as a bash statement in the current context of the
...
If you want bash shell command-processing, it's possible to go into an
python shell with the corresponding the command ``bash`` or ``shell``. It
